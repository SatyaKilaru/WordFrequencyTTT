...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const counts = Object.entries(wordMap).map(([word, count]) => ({ word, count }));
      counts.sort((a, b) => b.count - a.count);
      const topWords = counts.slice(0, 20);
  ...
  const options = {
    ...
    xaxis: {
      categories: wordCounts.map((item) => item.word),
    },
  };
      ...
      {wordCounts.length > 0 && (
        <div align="center">
          ...
          <section class="container">
            ...
            <button data-hover="click me!" onClick={exportData}><div>Export</div></button>
